...
const PortfolioComponent: React.FC<{ investmentData: Portfolio }> = ({ investmentData }) => {
 ...
 if (investmentData.investment.investmentDate){
  ...
  console.log(investmentData.manager.duration*7)
  chartObject = createMultiplicationObject(new Date(investmentData.investment.investmentDate).toISOString(), 
  investmentData.investment.amountDeposited*(investmentData.manager.percentageYield/(investmentData.manager.duration*7*100)));
 }
if (chartObject){
 processedData = {
      ...
      <Col xs={12} md={6} lg={3}>
        <div>
          <PortfolioCard
            ...
            mainText={investmentData.investment.amountDeposited!==0?`$${dailyEarningPercentage(investmentData.manager.percentageYield, investmentData.manager.duration * 7)}%` : '0%'}
            subText={investmentData.investment.amountDeposited!== 0 ? `capital grows by ${dailyEarningPercentage(investmentData.manager.percentageYield, investmentData.manager.duration * 7)}X daily` : 'No investment yet'}
          ...
          <PortfolioCard
            title={'Pay Out Date'}
            mainText={investmentData.investment.investmentDate !== null ?  formatEndDate(new Date(investmentData.investment.investmentDate).toISOString(), investmentData.manager.duration * 7) : 'N/A'}
            subText={investmentData.investment.amountDeposited! !== 0 ? `investment tenure of ${investmentData.manager.duration * 7} days` : 'No investment yet'}
            primaryBackground={true}
          />
        </div>
      </Col>
